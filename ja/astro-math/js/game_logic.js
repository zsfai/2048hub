...
function generateQA() {
  let questions = getLocal("game質問s");
...
// don't replace
let gameAsteroids = [読み込み中読み込み中読み込み中読み込み中読み込み中...document.querySelectorAll(".main__asteroid-container")];
...
function naviCaller() {
  if (this.dataset.value === "next") {
    ...
    updateLocal("game質問s", localjson[nextLevel][3]);
