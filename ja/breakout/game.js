...
Game = {
  ...
  loadSounds: function(cfg) {
    cfg = cfg || {};
    if (typeof サウンドManager == 'undefined') {
      var path = cfg.path || 'サウンド/サウンドmanager2-nodebug-jsmin.js';
      var swf  = cfg.swf  || 'サウンド/swf';
      window.SM2_DEFER = true;
      Game.loadScript(path, function() {
        window.サウンドManager = new SoundManager();
        サウンドManager.useHighPerformance = true;
        サウンドManager.useFastPolling = true;
        サウンドManager.url = swf;
        サウンドManager.defaultOptions.volume = 50; // shhh!
        サウンドManager.onready(function() {
          Game.loadSounds(cfg);
        });
        サウンドManager.beginDelayedInit();
      });
    }
    else {
      var サウンドs = [];
      for(var id in cfg.サウンドs) {
        サウンドs.push(サウンドManager.createSound({id: id, url: cfg.サウンドs[id]}));
      }
      if (cfg.onload)
        cfg.onload(サウンドs);
    }
