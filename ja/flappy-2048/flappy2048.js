...
var loadGameImage = function (n) {
 ...
 o.onload = function () {
  want_image_count--;
  if ((want_image_count == 0) && (want_サウンド_count == 0)) {
   gameLoaded();
  }
 }
 o.src = n;
...
wing.img = loadGameImage('wing.png');
...
wingfill.img = loadGameImage('wingfill.png');
...
logo.showing = true;
logo.hiding = false;
logo.img = loadGameImage('logo.png');
...
gameover.showing = false;
gameover.hiding = true;
gameover.img = loadGameImage('gameover.png');
...
c2s.showing = true;
c2s.hiding = false;
c2s.img = loadGameImage('clicktostart.png');
...
var sndo = {};
if (localStorage.サウンド === undefined) {
  localStorage.サウンド = 'true';
}
sndo.has_サウンド = localStorage.サウンド === 'true';
sndo.div = document.createElement('div');
sndo.div.style.backgroundImage = 'url("サウンドoff.png")';
...
function newCell () {
 ...
 div.style.lineHeight = cell_size+'px';
 div.style.position = 'absolute';
 div.style.textAlign = 'center';
 div.style.fontFamily = '"Clear Sans", "Helvetica Neue", Arial, sans-serif';
   ...
   if (game.started) {
    ...
    for (var i = walls.length-1; i>=0; i--) {
     ...
     for (var j = cells.length-1; j>=0; j--) {
      ...
      cell.vx = wall.vx;
      cell.x = wall.x;
      cell.y = (hh-88) - (cell.sty + mhv*30 + wall.vspeed*wall.dir*(wall.x - (wall.ww + 12*2) - bird.x))%mhv;
