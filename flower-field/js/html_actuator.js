function HTMLActuator() {
  ...
  this.score = 0;
  this.shareUrl = "https://2048hub.com/flower-field/";
...
HTMLActuator.prototype.message = function (metadata) {
  ...
  var bloomLine = names.length
    ? "Blooms this round: " + names.join(", ")
    : "No full blooms yet — every seed counts.";

  var highest = metadata.highestBloom
    ? FlowerConfig.getStage(metadata.highestBloom).name
    : "Seed";

  var notebookLine = metadata.sessionNewNotebook
    ? "Your notebook gained " + metadata.sessionNewNotebook + " new note" +
      (metadata.sessionNewNotebook > 1 ? "s" : "") + "."
    : "Your notebook is waiting for its next whisper.";
  ...
  if (p) {
    p.innerHTML =
      bloomLine + "<br><br>" +
      "<strong>Highest bloom:</strong> " + highest + "<br>" +
      notebookLine;
  }
...
HTMLActuator.prototype.drawShareCard = function (data) {
  ...
  ctx.fillText("Flower Field", 24, 40);

  ctx.fillStyle = "#7a8f6a";
  ctx.font = "16px Georgia, serif";
  ctx.fillText("I grew a little garden on 2048Hub.", 24, 72);

  ctx.fillStyle = "#4a5a3a";
  ctx.font = "bold 20px Georgia, serif";
  ctx.fillText("Highest bloom: " + data.highest, 24, 110);

  ctx.fillStyle = "#6a7a5a";
  ctx.font = "15px Georgia, serif";
  wrapText(ctx, "\"" + data.message + "\"", 24, 145, w - 48, 22);

  ctx.fillStyle = "#9aab8a";
  ctx.font="13px Georgia, serif";
  ctx.fillText("2048hub.com/flower-field", 24, h - 24);
};
...
HTMLActuator.prototype.shareResult = function () {
  var data = this.lastShareData || { highest: "Seed", message: "Keep tending your field.", blooms: [] };
  var text =
    "I grew a flower field on 2048Hub. My highest bloom is \"" + data.highest +
    "\", and it told me — \"" + data.message +
    "\". Come see what you can grow?\n" + this.shareUrl;

  if (navigator.share) {
    navigator.share({ title: "Flower Field", text: text, url: this.shareUrl }).catch(function () {});
    return;
  }
